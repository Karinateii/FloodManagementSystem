...
namespace NewLagosFloodDetectionSystem.Controllers
{
    ...
    public class CsvFileCityController : Controller
    {
        ...
        public CsvFileCityController(FloodDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        ...
        {
            var files = _context.CsvFilesCities.ToList();
            return View(files);
        }
            ...
            if (file != null)
            {
                if (file.Length > 0)
                {
                    ...
                    if (fileExtension == ".csv")
                    {
                        ...
                        var yearCheck = _context.CsvFilesCities.FirstOrDefault(x => x.Year ==  model.Year && x.CityId == model.CityId);
                        if(yearCheck == null)
                        {
                            _context.CsvFilesCities.Add(objfiles);
        ...
        {
            var predFiles = await _context.CsvFilesCities.FindAsync(id);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            ...
            _context.CsvFilesCities.Remove(predFiles);
